...
#These were kept for reference onlly
...
#' *ANSWER:*
#' 
#' listings %>% mutate(price_bin = cut(price, breaks = quantile(price))) %>% group_by(bedrooms, price_bin) %>% summarize(mean_rating = mean(review_scores_rating, na.rm = TRUE))
#' listings %>% mutate(price_bin = cut(price, breaks = quantile(price))) %>% group_by(bedrooms, price_bin) %>% summarize(mean_rating = mean(review_scores_rating, na.rm = TRUE), bb_rating = mean(review_scores_rating[neighbourhood_cleansed == "Back Bay"], na.rm = TRUE)) %>% select(bb_rating)
#' 
...
#' *ANSWER:*
#' to.plot %>%
#' ggplot(aes(x=mean_value_rating, y=mean_location_rating, color=factor(cancellation_policy))) +
#' geom_point() +
#' labs(x='Mean Value Rating', y='Mean Location Rating', title='Some kind of Plot by Cancellation Policy')
#' 
...
#' *ANSWER:*
#' 
#' reviews_neighborhood <- listings %>% select(neighbourhood, review_scores_cleanliness, review_scores_location, review_scores_value)
#' reviews_neighborhood %>% filter(!is.na(neighbourhood)) %>% filter(!is.na(review_scores_cleanliness)) %>% filter(!is.na(review_scores_location)) %>% filter(!is.na(review_scores_value)) 
#' reviews_mean <- reviews_neighorhood %>% 
#' 
#' 
#' More effeciently:
#' 
#' listings %>% gather(rating_type, rating, review_scores_cleanliness, review_scores_location, review_scores_value) %>%
#' group_by(neighbourhood_cleansed, rating_type) %>%
#' summarise(mean_rating = mean(rating, na.rm=TRUE)) %>%
#' ggplot(aes(neighbourhood_cleansed, mean)rating, fill=rating_type) + 
